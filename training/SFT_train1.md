...
**ROUGH**: 
- 요약 / 번역 품질 평가에 주로 사용.
- 재현율(recall)에 중점을 두고 있음.
- 생성된 텍스트가 기준 텍스트와 생성된 텍스트의 유사성에 집중.
**ROUGH 1**: 시스템 요약본과 참조 요약본 간 겹치는 unigram의 수를 보는 지표.
**ROUGH 2**: 시스템 요약본과 참조 요약본 간 겹치는 bigram의 수를 보는 지표.
**ROUGH-L**: LCS 기법을 이용해 최장 길이로 매칭되는 문자열을 측정.